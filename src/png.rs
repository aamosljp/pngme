...
impl Png {
    ...
    fn from_chunks(chunks: Vec<Chunk>) -> Png {
        Png {
            header: Png::STANDARD_HEADER.to_vec(),
            chunks,
        }
    }
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        ...
        bytes.extend_from_slice(&self.header.iter().copied().collect::<Vec<u8>>());
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, Self::Error> {
        ...
        if header != Png::STANDARD_HEADER {
            return Err(format!(
                "Invalid PNG header: expected {}, got {}",
                Png::STANDARD_HEADER.iter().map(|b| b.to_string()).collect::<Vec<String>>().join(", "),
                header.iter().map(|b| b.to_string()).collect::<Vec<String>>().join(", ")
            )
            .into());
        }
